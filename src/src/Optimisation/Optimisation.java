...
public class Optimisation {
	...
	private ArrayList<Integer> resultQ;
	private ArrayList<Double> resultP;

	public ArrayList<Integer> getResultQ() {
		return resultQ;
	}

	

	public ArrayList<Double> getResultP() {
		return resultP;
	}


	public void optimise(double eAmont, double max_flow) {
		resultQ = new ArrayList<Integer>();
		...
		for (int i = 0; i < NB_LINE; i++) {
			...
			for (int j = 1; j < NB_LINE + 1; j++) {
				...
				if (j < i + 2) {
					stage4[i][j] = (calculatedData.power[3][j - 1] + stage5[i - j + 1][1]); // f4(i,j-1) = p4(j-1) +
																							// f5*(i-j+1)
		...
		for (int i = 0; i < NB_LINE; i++) {
			...
			for (int j = 1; j < NB_LINE + 1; j++) {
				...
				if (j < i + 2) {
					stage3[i][j] = (calculatedData.power[2][j - 1] + stage4[i - j + 1][NB_LINE + 1]); // f4(i,j-1) =
																										// p4(j-1) +
																										// f5*(i-j+1)
		...
		for (int i = 0; i < NB_LINE; i++) {
			...
			for (int j = 1; j < NB_LINE + 1; j++) {
				...
				if (j < i + 2) {
					stage2[i][j] = (calculatedData.power[1][j - 1] + stage3[i - j + 1][NB_LINE + 1]); // f4(i,j-1) =
																										// p4(j-1) +
																										// f5*(i-j+1)
		...
		for (int j = 1; j < NB_LINE + 1; j++) {
			...
			stage1[0][j] = (calculatedData.power[0][j - 1] + stage3[NB_LINE - j][NB_LINE + 1]); // f4(i,j-1) = p4(j-1) +
																								// f5*(i-j+1)
		 ...
		 * 
		 * NumberFormat nf = new DecimalFormat("0.0"); System.out.println("stage 5 : ");
		 * for(int i=0;i<NB_LINE;i++) { System.out.print("line " + i +": "); for(int
		 * j=0;j<3;j++) { System.out.print(nf.format(stage5[i][j])+" | "); }
		 * System.out.println("");
		 * 
		 * }
		 * 
		 * System.out.println("stage 4 : "); for(int i=0;i<NB_LINE;i++) {
		 * System.out.print("line " + i +": "); for(int j=0;j<NB_LINE+3;j++) {
		 * System.out.print(nf.format(stage4[i][j])+" | "); } System.out.println("");
		 * 
		 * }
		 * 
		 * System.out.println("stage 3 : "); for(int i=0;i<NB_LINE;i++) {
		 * System.out.print("line " + i +": "); for(int j=0;j<NB_LINE+3;j++) {
		 * System.out.print(nf.format(stage3[i][j])+" | "); } System.out.println(""); }
		 * 
		 * 
		 * System.out.println("stage 2 : "); for(int i=0;i<NB_LINE;i++) {
		 * System.out.print("line " + i +": "); for(int j=0;j<NB_LINE+3;j++) {
		 * System.out.print(nf.format(stage2[i][j])+" | "); } System.out.println(" ");
		 * 
		 * }
		 * 
		 * 
		...
		System.out.println("Valeurs en entrée : eAmont=" + formatter.format(eAmont) + "; Débit à répartir= "
				+ SettingValue.MAX_FLOW);
		System.out.println("Optimum : ");
		for (int i = 0; i < 5; i++) {
			int Qi = optimum[i] * SettingValue.DISCRETISATION;
			double Pi=calculatedData.getPower(Qi, i + 1, eAmont);
			System.out.print("turbine " + (i + 1) + ": ");
			System.out.print(Qi + " m3/s => ");
			System.out.println(formatter.format(Pi) + " MW");
			resultQ.add(Qi);
			resultP.add(Pi);
		}
